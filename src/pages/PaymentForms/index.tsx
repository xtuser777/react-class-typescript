...
export class PaymentForms extends Component<unknown, IComponentState> {
  constructor(props: unknown) {
    super(props);

    this.state = { data: [], forms: [], filter: '', orderBy: '1' };
  }

  getData = async () => {
    const data = await new PaymentForm().get();
    this.setState({ data, forms: data });
  };
  ...
  filterData = (orderBy: string) => {
    ...
    let filteredData: IPaymentForm[] = [...data];
  ...
  handleOrderChange = (e: ChangeEvent<HTMLInputElement>) => {
    this.setState({ orderBy: e.target.value, forms: this.filterData(e.target.value) });
  };

  handleFilterClick = () => {
    this.setState({ forms: this.filterData(this.state.orderBy) });
  };
  ...
  remove = async (id: number) => {
    ...
    if (response) {
      ...
      if (await form.delete()) {
        ...
        this.setState({ data: newData, forms: newForms });
      }
    }
  };

  render() {
    const { forms, filter, orderBy } = this.state;
    return (
      <>
